...
{
	/// <summary>
	/// Each <see cref="Interface">Interface</see> contains data items which are used to communicate information required to execute the <i>interface</i>. When these data items are read by another piece of equipment, that piece of equipment can then determine the actions that it may take based upon that data.  <see cref="InterfaceState">InterfaceState</see> is a data item specifically defined for <i>interface</i>s.  It defines the operational state of the <i>interface</i>.  This is an indicator identifying whether the <i>interface</i> is functioning or not. See <see cref="InterfaceState">InterfaceState</see> for complete semantic details.  Some data items <b>MAY</b> be directly associated with the <see cref="Interface">Interface</see> element and others will be organized by a <see cref="References">References</see> element. It is up to an implementer to determine which additional data items are required for a particular <i>interface</i>.  ### Specific Data Items for the Interaction Model for Interface  A special set of data items have been defined to be used in conjunction with <see cref="Interface">Interface</see>. They provide information from a piece of equipment to <i>request</i> a service to be performed by another associated piece of equipment; and for the associated piece of equipment to indicate its progress in performing its <i>response</i> to the <i>request</i> for service.  .  Many of the data items describing the services associated with an <i>interface</i> are paired to describe two distinct actions – one to <i>request</i> an action to be performed and a second to reverse the action or to return to an original state.  For example, a <see cref="DoorInterface">DoorInterface</see> will have two actions <see cref="OpenDoor">OpenDoor</see> and <see cref="CloseDoor">CloseDoor</see>. An example of an implementation of this would be a robot that indicates to a machine that it would like to have a door opened so that the robot could extract a part from the machine and then asks the machine to close that door once the part has been removed.   When these data items are used to describe a service associated with an <i>interface</i>, they <b>MUST</b> have one of the following two <c>subType</c> elements: <c>REQUEST</c> or <c>RESPONSE</c>.  These <b>MUST</b> be specified to define whether the piece of equipment is functioning as the <i>requester</i> or <i>responder</i> for the service to be performed.  The <i>requester</i> <b>MUST</b> specify the <c>REQUEST</c> <c>subType</c> for the data item and the <i>responder</i> <b>MUST</b> specify a corresponding <c>RESPONSE</c> <c>subType</c> for the data item to enable the coordination between the two pieces of equipment.     These data items and their associated <c>subType</c> provide the basic structure for implementing the <i>interaction model</i> for an <i>interface</i> and are defined in the following sections.  {{figure(Request State Machine)}} and {{figure(Response State Machine)}} show possible state transitions for a <i>request</i> and <i>response</i> respectively. The state machine diagrams provide the permissible values of the <i>observation</i>s for the <see cref="DataItem">DataItem</see> types listed in this section.  ![Request State Machine](figures/Request%20State%20Machine.png "Request State Machine"){: width="0.8"}  ![Response State Machine](figures/Response%20State%20Machine.png "Response State Machine"){: width="0.8"} 
	/// </summary>
	...
	public class DataItemTypesForInterfacePackage : IPackage
	{
		...
		/// <inheritdoc />
		public string Name => "DataItemTypesForInterface";

		/// <inheritdoc />
		public string Summary => @"/// Each <see cref=""Interface"">Interface</see> contains data items which are used to communicate information required to execute the <i>interface</i>. When these data items are read by another piece of equipment, that piece of equipment can then determine the actions that it may take based upon that data.  <see cref=""InterfaceState"">InterfaceState</see> is a data item specifically defined for <i>interface</i>s.  It defines the operational state of the <i>interface</i>.  This is an indicator identifying whether the <i>interface</i> is functioning or not. See <see cref=""InterfaceState"">InterfaceState</see> for complete semantic details.  Some data items <b>MAY</b> be directly associated with the <see cref=""Interface"">Interface</see> element and others will be organized by a <see cref=""References"">References</see> element. It is up to an implementer to determine which additional data items are required for a particular <i>interface</i>.  ### Specific Data Items for the Interaction Model for Interface  A special set of data items have been defined to be used in conjunction with <see cref=""Interface"">Interface</see>. They provide information from a piece of equipment to <i>request</i> a service to be performed by another associated piece of equipment; and for the associated piece of equipment to indicate its progress in performing its <i>response</i> to the <i>request</i> for service.  .  Many of the data items describing the services associated with an <i>interface</i> are paired to describe two distinct actions – one to <i>request</i> an action to be performed and a second to reverse the action or to return to an original state.  For example, a <see cref=""DoorInterface"">DoorInterface</see> will have two actions <see cref=""OpenDoor"">OpenDoor</see> and <see cref=""CloseDoor"">CloseDoor</see>. An example of an implementation of this would be a robot that indicates to a machine that it would like to have a door opened so that the robot could extract a part from the machine and then asks the machine to close that door once the part has been removed.   When these data items are used to describe a service associated with an <i>interface</i>, they <b>MUST</b> have one of the following two <c>subType</c> elements: <c>REQUEST</c> or <c>RESPONSE</c>.  These <b>MUST</b> be specified to define whether the piece of equipment is functioning as the <i>requester</i> or <i>responder</i> for the service to be performed.  The <i>requester</i> <b>MUST</b> specify the <c>REQUEST</c> <c>subType</c> for the data item and the <i>responder</i> <b>MUST</b> specify a corresponding <c>RESPONSE</c> <c>subType</c> for the data item to enable the coordination between the two pieces of equipment.     These data items and their associated <c>subType</c> provide the basic structure for implementing the <i>interaction model</i> for an <i>interface</i> and are defined in the following sections.  {{figure(Request State Machine)}} and {{figure(Response State Machine)}} show possible state transitions for a <i>request</i> and <i>response</i> respectively. The state machine diagrams provide the permissible values of the <i>observation</i>s for the <see cref=""DataItem"">DataItem</see> types listed in this section.  ![Request State Machine](figures/Request%20State%20Machine.png ""Request State Machine""){: width=""0.8""}  ![Response State Machine](figures/Response%20State%20Machine.png ""Response State Machine""){: width=""0.8""} ";
